...
rnd_cols <- colnames(df[, grep(pattern="dmri_rsirndgm.*\\change_score", colnames(df))])
...
write.csv(complete_df, paste(PROJ_DIR, DATA_DIR,"complete_data.csv", sep="/"), row.names = T)
...
for(i in 1:length(unique(colnames(df_rsi)))){
...
library("writexl")
write.csv(scree_df,paste(PROJ_DIR, OUTP_DIR, paste("rnd-scree_values_", resPerm4PLSC$pOmnibus, ".csv", sep = ""), sep = "/"))
...
my.scree
dev.copy(png, paste(PROJ_DIR, FIGS_DIR, 'rnd-PLSC_scree.png', sep="/"))
dev.off()
...
counts = F)
dev.copy(png, paste(PROJ_DIR, FIGS_DIR, 'rnd-LD1_perm_hist.png', sep="/"))
dev.off()
...
Latent dimension ",zeDim),
  ylab = "Number of samples",
  counts = F)
dev.copy(png, paste(PROJ_DIR, FIGS_DIR, 'rnd-LD2_perm_hist.png', sep="/"))
dev.off()
...
plot1
dev.copy(png, paste(PROJ_DIR, FIGS_DIR, 'rnd-LD1_factor_map.png', sep="/"))
dev.off()
...
plot2
dev.copy(png, paste(PROJ_DIR, FIGS_DIR, 'rnd-LD2_factor_map.png', sep="/"))
dev.off()
...
p.plot
dev.copy(png, paste(PROJ_DIR, FIGS_DIR, 'rnd-region_loading_lv1xlv2.png', sep="/"))
dev.off()
...
q.plot
dev.copy(png, paste(PROJ_DIR, FIGS_DIR, 'rnd-apsource_loading_lv1xlv2.png', sep="/"))
dev.off()
...
plot_P.data1 
dev.copy(png, paste(PROJ_DIR, FIGS_DIR, 'rnd-LD1_brain_loadings.png', sep="/"))
dev.off()
...
plot_Q.data2
dev.copy(png, paste(PROJ_DIR, FIGS_DIR, 'rnd-LD1_apsource_loadings.png', sep="/"))
dev.off()
...
#library("writexl")
#write_xlsx(df_loadings,paste(PROJ_DIR, OUTP_DIR, "plsc_loadings_rnd.csv", sep = "/"))
...
) + ggtitle(paste0('Latent dimension ', laDim), subtitle = 'Grey Matter Anisotropic Intracellular Diffusion Saliences (Loadings)')
...
ba001.BR1.I
dev.copy(png, paste(PROJ_DIR, FIGS_DIR, 'rnd-LD1_brain_saliences.png', sep="/"))
dev.off()
ba002.BR1.J 
dev.copy(png, paste(PROJ_DIR, FIGS_DIR, 'rnd-LD1_apsource_saliences.png', sep="/"))
dev.off()
...
ba001.BR1.I
dev.copy(png, paste(PROJ_DIR, FIGS_DIR, 'rnd-LD2_brain_saliences.png', sep="/"))
dev.off()
ba002.BR1.J 
dev.copy(png, paste(PROJ_DIR, FIGS_DIR, 'rnd-LD2_apsource_saliences.png', sep="/"))
dev.off()
