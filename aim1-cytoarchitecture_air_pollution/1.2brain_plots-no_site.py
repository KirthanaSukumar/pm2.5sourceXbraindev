...
for sig_dim in sig_dims:
    ...
    plotting.plot_img_on_surf(
            ...
            output_file=join(PROJ_DIR, FIGS_DIR, f'rnd-no_site-{loadings.name}.png')
        )

rnd_brain_loadings.to_csv(join(PROJ_DIR, OUTP_DIR, "rnd-no_site-brain_P-region_names.csv"))


rnd_brain_loadings = pd.read_csv(join(PROJ_DIR, OUTP_DIR, "rni-no_site-brain_P.csv"), header=0, index_col=0)
...
for sig_dim in sig_dims:
    ...
    plotting.plot_img_on_surf(
            ...
            symmetric_cbar=True,
            kwargs={'bg_on_data':False, 'alpha': 1, 'avg_method': 'max'},
            output_file=join(PROJ_DIR, FIGS_DIR, f'rni-no_site-{loadings.name}.png')
        )

rnd_brain_loadings.to_csv(join(PROJ_DIR, OUTP_DIR, "rni-no_site-brain_P-region_names.csv"))
