...
bmi_df <- subset(bmi_df, eventname=='2_year_follow_up_y_arm_1')
rownames(bmi_df) <- bmi_df$src_subject_id

rni_cols <- colnames(brain[, grep(pattern = "dmri_rsirni.*",
                                  colnames(brain))])
...
df1 <- cbind(df[ppts_in_common,], brain[ppts_in_common,], bmi_df[ppts_in_common,])
temp_df <- df1[, c(
  rni_cols, 
  rsfmri_cols, 
  cov_cols
...
png(paste(proj_dir, figs_dir, "rni-brain_residuals-2yfu.png", sep = "/"),
...
png(paste(proj_dir, figs_dir, "rni-rsvar_pollution_residuals-2yfu.png", sep = "/"),
    width = 5, height = 5, res = 600, units = "in")
...
png(paste(proj_dir, figs_dir, "rniXrsvar_corr-2yfu.png", sep = "/"),
...
#png(paste(proj_dir, figs_dir, "rniXrsvar_ckorr-2yfu.png", sep = "/"),
#    width = 10, height = 5, res = 600, units = "in")
...
png(paste(proj_dir, figs_dir, "rniXrsfc-PLSC_scree-2yfu.png", sep = "/"),
    width = 5, height = 5, res = 600, units = "in")
...
png(paste(proj_dir, figs_dir, "rniXrsfc-LD1_factor_map-2yfu.png", sep = "/"),
...
ggsave(plot1, file=paste(proj_dir, figs_dir, "rniXrsfc-LD1_factor_map-2yfu.png", 
                         sep="/"),
...
ggsave(plot2, file=paste(proj_dir, figs_dir, "rniXrsfc-LD2_factor_map-2yfu.png", 
                         sep="/"),
       width = 10, height = 10, units = "in", dpi=600)
...
ggsave(p.plot, file=paste(proj_dir, figs_dir, "rniXrsfc-brain_factors-2yfu.png", 
                          sep="/"), 
       width = 10, height = 10, units = "in", dpi=600)
...
ggsave(q.plot, file=paste(proj_dir, figs_dir, "rniXrsfc_factors-2yfu.png", 
                          sep="/"),
       width = 10, height = 10, units = "in", dpi=600)
...
plot_P.data1
ggsave(plot_P.data1, file=paste(proj_dir, figs_dir, "rni-brain_P-2yfu.png", 
                                sep = "/"), 
       width = 10, height = 10, units = "in", dpi=600)
...
plot_Q.data2
ggsave(plot_Q.data2, file=paste(proj_dir, figs_dir, "rni-rsfc_Q-2yfu.png", 
                                sep = "/"), 
       width = 10, height = 10, units = "in", dpi=600)
...
if (resPerm4PLSC$pOmnibus < 0.01) {
  for (i in seq_along(resPerm4PLSC$pEigenvalues)) {
    if (resPerm4PLSC$pEigenvalues[i] < 0.01) {
      laDim <- i
      print(laDim)
      png(paste(proj_dir, figs_dir, paste("rniXrsfc-LD", 
                                          i, 
                                          "_brain_saliences-2yfu.png", 
                                          sep=""), sep = "/"),
          width = 10, height = 10, res = 600, units = "in")
      ...
      ba001.BR1.I
      ggsave(ba001.BR1.I, file=paste(proj_dir, figs_dir, paste("rniXrsfc-LD", 
                                                               i, 
                                                               "_brain_saliences-2yfu.png", 
                                                               sep=""), sep = "/"), 
             width = 10, height = 10, units = "in", dpi=600)
      ...
      ba001.BR1.J
      ggsave(ba001.BR1.J, file=paste(proj_dir, figs_dir, paste("rniXrsfc-LD", 
                                                               i, 
                                                               "_rsfc_saliences-2yfu.png", 
                                                               sep=""), sep = "/"), 
             width = 10, height = 10, units = "in", dpi=600)
